...
The app allows users to register, list dishes for sale, and browse available offers. It is designed to be simple and accessible: find a meal, place an order, pick it up from down the hall.
...
the landing page can be browsed without an account, registration and login appear when a user tries to place an order, and the profile page (with editing) is available to both students and hobby cooks.

Hungry student screens cover the full journey from browsing the feed, viewing meal details, confirming a reservation (cash or PayPal), and tracking orders on the dashboard. What was dropped: the post-pickup rating step never got built. There's no rating table, model, or screen anywhere in the app.

Hobbycook screens kept posting a dish (price, portions, pickup time) and managing active offers (edit/delete on the dashboard). **What was dropped:**

`Weekly meal planning` - hobby cooks post one dish at a time; there's no calendar or planning view for the week.

`Kitchen cleanliness photo proof` - the kitchen_proof table (before/after photo with handwritten timestamp) was modeled early on but never got a model, route, or upload screen. It's been removed from the schema since it was dead weight.

`Order notifications` - same story: a notification table existed but no logic was ever built around it, so it's gone too.

**Added, not in the original scope:**

`Automatic icon detection` - dishes get a category icon (pizza, pasta, curry, etc.) picked automatically by matching keywords in the name/description/ingredients, no manual step needed.

`Tags (tag/dish_tag)` - a lightweight tagging structure (e.g. "Italian", "comfort food") was added at the data level, though it isn't yet surfaced in the UI.

`message table/model` - kept in the schema (sender, receiver, linked order) as groundwork for messaging between students and hobby cooks, but no chat screen was built on top of it yet.
